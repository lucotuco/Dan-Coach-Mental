...
export default function GuidedAudioSession({
  title,
  scriptTitle,
  scriptParagraphs,
  audioModule,
  ...
  const progressPercentage = playerStatus?.duration
    ? Math.min(1, Math.max(0, (playerStatus?.currentTime ?? 0) / playerStatus.duration))
    : 0;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const meditationAsset = Asset.fromModule(audioModule);
  ...
  const handlePlayPausePress = () => {
    ...
    const finished =
      playerStatus?.didJustFinish || (!!playerStatus?.duration && playerStatus.currentTime >= playerStatus.duration);
  ...
  return (
    <>
      ...
      <View style={styles.screen}>
        <View style={styles.heroCard}>
          ...
          {isPlaying ? (
            <View style={styles.playbackControls}>
              <Pressable accessibilityRole="button" style={styles.skipButton} onPress={handleSkipBack}>
                <Ionicons name="play-back" size={30} color="#031355" />
                <Text style={styles.skipLabel}>-10s</Text>
              </Pressable>

              <Pressable accessibilityRole="button" onPress={handlePlayPausePress} style={styles.controlButton}>
                <Ionicons name="pause" size={28} color="#fff" />
              </Pressable>

              <Pressable accessibilityRole="button" style={styles.skipButton} onPress={handleSkipForward}>
            ...
            <View style={styles.progressContainer}>
              <View style={styles.progressHeader}>
                ...
                <Text style={styles.progressTime}>
                  ...
                  {playerStatus?.duration ? ` / ${formatTime(playerStatus?.duration)}` : ''}
                </Text>
              </View>
        ...
        <View style={styles.scriptContainer} onLayout={(event) => setScriptContainerHeight(event.nativeEvent.layout.height)}>
          <ScrollView
            ref={scriptScrollRef}
